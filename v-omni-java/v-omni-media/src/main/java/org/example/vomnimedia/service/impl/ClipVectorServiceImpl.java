...
public class ClipVectorServiceImpl implements VectorService, AutoCloseable {
    ...
    public void init() {
        try {
            ...
            log.info("✅ 集成化 CLIP ONNX 模型加载成功 (Vision + Text + UserTower)");
    ...
    public float[] getVector(List<byte[]> imageBytesList) throws Exception {
        ...
        float[] dummyBehavior = new float[VECTOR_DIM]; // 长度为 1 的行为序列
        ...
        try {
            ...
            container.put("attention_mask", OnnxTensor.createTensor(env, LongBuffer.wrap(new long[MAX_TEXT_LENGTH]), new long[]{1, MAX_TEXT_LENGTH}));
    ...
    public float[] getTextVector(String text) throws Exception {
        ...
        try {
            ...
            try (OrtSession.Result results = session.run(container)) {
                // 注意：根据 Python 导出设置，输出为 [Batch, Dim]，这里返回第一行
    ...
    private float[] preprocess(BufferedImage img) {
        ...
        for (int y = 0; y < IMAGE_SIZE; y++) {
            for (int x = 0; x < IMAGE_SIZE; x++) {
                ...
                // CLIP 要求的 Planar 格式: RRR...GGG...BBB...
